            ...
            <tr v-for="goods in $store.getters['cart/validList']" :key="goods.skuId">
              ...
              <td class="tc">
                ...
                <p v-if="goods.price-goods.nowPrice > 0">
                  比加入时降价
                  <span class="red">&yen;{{goods.price - goods.nowPrice}}</span></p>
              </td>
              <td class="tc">
...
export default {
  ...
  setup () {
    ...
    // 批量删除已选择中商品, 也支持清空无效商品
    const batchDeleteCart = (isClear) => {
      Confirm({ text: `你是否确认删除${isClear ? '失效' : '选中'}的商品` }).then(() => {
    ...
    // 修改规格 传入两个ID， 一个是旧的sku信息， 一个是新的sku信息
    ...
    const checkout = () => {
      ...
      if (store.getters['cart/selectedList'].length === 0) {
        return Message({ text: '至少要选中一件商品' })
      }
      Confirm({ text: '下单结束需要登录，确定现在去登录码' }).then(() => {
        router.push('/member/checkout')
      }).catch(e => {})
    }

    return { checkOne, checkAll, deleteCart, batchDeleteCart, updateCount, updataCartSku, checkout }
  }
}
</script>
