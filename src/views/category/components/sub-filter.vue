<!--二级类目-筛选区展示-->
<template>
   <div class="sub-filter" v-if="filterData && filterLoading === false"><!-- v-if="filterData"判断是否有数据-->
     <div class="item">
...
export default {
  ...
  setup (props, { emit }) {
    ...
    const filterLoading = ref(false)// 显示状态
    watch(() => route.params.id, (newVal) => {
      // 变化后的ID有值，并且 处在二级类目路由下 等于rout.path路径
      if (newVal && `/category/sub/${newVal}` === route.path) {
        filterLoading.value = true// 显示状态
        // 发请求获取数据
        findTopCategoryFilter(route.params.id).then(data => {
          ...
          filterLoading.value = false // 显示状态
          console.log('筛选条件数据', data.result)
        })
      }
    }, { immediate: true })
