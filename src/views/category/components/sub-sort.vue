...
<template>
  <div class='sub-sort'>
    ...
    <div class="check">
      <XtxCheckbox @click="changeChenk" v-model="sortParams.inventory">仅显示有货商品</XtxCheckbox>
      <XtxCheckbox @click="changeChenk" v-model="sortParams.onlyDiscount">仅显示特惠商品</XtxCheckbox>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  setup (props, { emit }) {
    ...
    const sortParams = reactive({
      ...
      sortField: null, // 排序字段，取值范围：publishTime, orderNum, evaluateNum, price
    ...
    const changeSort = (sortField) => {
      if (sortField === 'price') {
        ...
        // 如果排序未改变停止逻辑
        if (sortParams.sortField === sortField) { // 可有可无
          return
        }
        sortParams.sortField = sortField
    ...
    return { sortParams, changeSort, changeChenk }
  }
}
</script>
