...
export default {
  ...
  setup () {
    ...
    const menuList = computed(() => {
      // 得到9个分类，且每个1级分类下的子分类只要2个
      const list = store.state.category.list.map(item => {
        return {
          id: item.id,
          name: item.name,
          // 判断有没有 children 如果有就取children下的前面两个
    ...
    // 得到弹出层的推荐商品数据
    const categoryId = ref(null) // 鼠标移入哪个数据就记录哪个数据的id
    const currCategory = computed(() => {
      return menuList.value.find(item => item.id === categoryId.value) // 当前id 等于 categoryId 记录的id时,我就拿到了当前所对应的分类id
    })

    // 获取品牌数据, 尽量不要使用async在setup(){}上
    findBrand().then(data => {
      brand.brands = data.result
      console.log('品牌', data.result)
    })
